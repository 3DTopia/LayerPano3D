...
def main():
    ...
    parser.add_argument('--load-from', default='/mnt/petrelfs/yangshuai/4drender/Text-to-4Dscene/load/depth_anything_v2_vitl.pth', type=str, 
                        help='Path to the pre-trained model weights.')
    parser.add_argument('--max-depth', default=20, type=float,
                        help='Maximum depth value for the depth map.')
    parser.add_argument('--img-path', type=str, default='/mnt/petrelfs/yangshuai/4drender/Text-to-4Dscene/Depth_Anything_V2/assets/examples/demo01.jpg',
    ...
    for k, filename in enumerate(filenames):
        ...
        depth = np.full(pred.shape, 0, np.float64)

        valid = np.abs(pred) > 0.01
        print(pred[valid].min(), pred[valid].max())

        valid_img = Image.fromarray((valid * 255).astype(np.uint8))
        valid_img.save(f"{args.outdir}_mask.png")
        depth[valid] = 100 / (pred[valid])
        print(depth[valid].min(), depth[valid].max())
